...
fn parse_toml(input: &str) -> TomlValue {
    toml::from_str(input).expect("checked-in TOML must parse")
}
...
fn authority_lock_is_canonical_and_reviewed() {
    ...
    assert_eq!(middleware["revision"], "a99b7609200c076bedab58d14581d64b57c04387");
    assert_eq!(middleware["stackTypespec"], "contracts/typespec/main.tsp");
    assert_eq!(middleware["stackJsonSchema"], "contracts/json-schema/middleware-stack.schema.json");
    ...
    let auth = parse_toml(SHARED_AUTH);
    assert_eq!(auth["compatibility"]["commit"].as_str(), shared_auth["revision"].as_str());
}

#[test]
fn api_role_and_shared_policies_are_structurally_fail_closed() {
    let middleware = parse_toml(MIDDLEWARE);
    let rate_limit = parse_toml(RATE_LIMIT);
    let lru = parse_toml(LRU);
    let stack = json(STACK);
    assert_eq!(middleware["repository_mode"].as_str(), Some("server-only"));
    assert_eq!(middleware["default_target"].as_str(), Some("api"));
    ...
    assert!(capabilities.iter().any(|v| v.as_str() == Some("idempotency")));
    assert!(!reps.iter().any(|v| v.as_str() == Some("text/html")));
    assert_eq!(settings["idempotency"]["enabled"].as_bool(), Some(true));
    let methods = settings["idempotency"]["requiredMethods"].as_array().expect("idempotency methods").iter().map(|v| v.as_str().expect("method string")).collect::<Vec<_>>();
    assert_eq!(methods, vec!["POST", "PUT", "PATCH"]);
...
fn rate_limit_manifest_matches_middleware_stack() {
    ...
    let policy = policies.iter().find(|p| p["policyId"].as_str() == Some(policy_id)).expect("default policy exists");
    ...
    assert_eq!(stack_policy["capacity"].as_i64(), policy["capacity"].as_integer());
...
fn canonical_shared_auth_config_has_no_authored_alias() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().expect("template-contracts must have a repository parent");
