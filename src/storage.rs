...
impl ArtifactStore {
    ...
    fn local_path(dir: &std::path::Path, key: &str) -> PathBuf {
        // Canonical keys are content-addressed (`artifacts/<sha>.<ext>`).
        // Guessable aliases (`github/…`, `packages/…`) are also server-built
        // from verified publish metadata, never copied from client input.
        dir.join(key)
    }
...
impl ArtifactStore {
    ...
    pub async fn put_public_aliases(
        ...
        match self {
            ...
            Self::Local { .. } | Self::S3 { .. } => {
                for key in keys {
                    self.put(key, bytes.clone(), content_type).await?;
                }
                Ok(())
            }
        }
