...
pub async fn get_declared_graph(
    ...
    let org_row = find_org(&state, &org_slug).await.map_err(|_| graph_not_found())?;
    ...
    respond(&headers, format, &document, &filename_stem(&org_slug, &name, &ver))
}
...
fn declared_document(
    ...
    let runtime = manifest.dependencies.iter().map(|kv| (kv, DependencyKind::Runtime));
    let build = manifest
        .build_dependencies
        .iter()
...
fn write_toml(value: &Value) -> ApiResult<String> {
    ...
    for (key, child) in root {
        ...
        out.push_str(&format!("\n[{}]\n", json_string(key)));
        let Value::Object(table) = child else { continue };
...
"#,
    ...
    fn yaml_stays_inside_the_safe_subset() {
        let yaml = String::from_utf8(encode(&sample_document(), DependencyGraphFormat::Yaml).unwrap())
            .unwrap();
        for forbidden in ['&', '*', '!'] {
            assert!(
    ...
    fn accept_and_format_conflict_is_not_silently_resolved() {
        ...
        assert_eq!(
            resolve_format(&accept("application/vnd.zpkg.dependency-graph.v1+toml"), None).unwrap(),
            DependencyGraphFormat::Toml
        );
        assert_eq!(
    ...
    fn strong_validators_match_only_the_same_representation() {
        ...
        assert_eq!(yaml.headers().get(DEPENDENCY_GRAPH_DIGEST_HEADER), Some(&digest));
    ...
    fn convenience_renderings_declare_they_are_not_authoritative() {
        ...
        for format in [DependencyGraphFormat::Dot, DependencyGraphFormat::Mermaid] {
            let rendered =
                String::from_utf8(encode(&document, format).unwrap()).unwrap();
